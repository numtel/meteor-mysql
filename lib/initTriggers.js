...
initTriggers = function(list){
  ...
  _.each(updatedTables, function(table){
    ...
    _.each(triggerDefs, function(def, i){
      ...
      if(conn._binlogFile !== undefined){
        // Binlog Tailing
        conditionString += 'CALL ' + escId(conn._tempTable + '_up') +
                            '(' + esc(updateKey) + ');';
      }else if(conn._updatedKeys !== undefined){
        // UDF transmission
    ...
    _.each(TRIGGER_EVENTS, function(event){
      ...
      createTrigger(conn, table, body, event, name);
    });
  });
};
