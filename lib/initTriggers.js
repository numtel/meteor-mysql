...
initTriggers = function(list){
  ...
  // TODO place body inside function, allowing existing triggers to simply be
  // prepended instead of wiped out!
  ...
  _.each(updatedTables, function(table){
    ...
    _.each(triggerDefs, function(def, i){
      ...
      if(conn._updatedKeys !== undefined){
        ...
        conditionString += [
          'SET @' + escId(conn._updateVar) + ' = (',
            'SELECT `update` FROM ' + escId(conn._updateTable),
            'ORDER BY `update` DESC LIMIT 1);',
          'UPDATE ' + escId(conn._updateTable),
            'SET `update`= @' + escId(conn._updateVar) + ' + 1',
