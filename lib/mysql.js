...
var Bintail = Npm.require('bintail');
...
var mysqlMethods = {
  ...
  initBinlogTail: function(filename, tableName){
    var self = this;
    self._tempTable = tableName = tableName || 'meteor_update_log';
    self._updatedKeys = [];
    self._binlogFile = filename;

    var tail = new Bintail(filename);
    var match = self._escapeQueryFun(function(esc, escId){
      return 'UPDATE ' + esc(tableName);
    });
    tail.on('data', function(chunk){
      chunk = String(chunk).split(match);
      console.log(chunk);
    });

    self.queryEx(function(esc, escId){
      return [
        'CREATE TEMPORARY TABLE IF NOT EXISTS ' + escId(tableName) + ' (',
    ...
    self.queryEx(function(esc, escId){
      return 'DROP PROCEDURE IF EXISTS ' + escId(tableName + '_up')
    });
    self.queryEx(function(esc, escId){
      return [
        'CREATE PROCEDURE ' + escId(tableName + '_up'),
        '(upkey INT(10) UNSIGNED)',
        'NOT DETERMINISTIC',
        'UPDATE ' + escId(tableName),
        'SET `id`=upkey WHERE `id`=1;'
      ].join('\n');
    });
  },
