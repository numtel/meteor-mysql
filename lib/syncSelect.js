...
var updateSubscription = function(){
  ...
  _.each(rows, function(row, index){
    if(self.data.length - 1 < index){
      self.subscription.added(self.subscription._name,
        self.subscription._subscriptionId + ':' + index, row);
      self.data[index] = row;
    }else if(JSON.stringify(self.data[index]) !== JSON.stringify(row)){
      self.subscription.changed(self.subscription._name,
        self.subscription._subscriptionId + ':' + index, row);
      self.data[index] = row;
    }
  });
  if(self.data.length > rows.length){
    for(var i = self.data.length - 1; i >= rows.length; i--){
      self.subscription.removed(self.subscription._name,
        self.subscription._subscriptionId + ':' + i);
    }
