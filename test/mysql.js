...
Tinytest.add('numtel:mysql - queryEx with string', function(test){
  var result = db.queryEx('select 1 + 1 as solution');
  test.equal(result[0].solution, 2);
});

Tinytest.add('numtel:mysql - queryEx with function', function(test){
  var value = 7;
...
Tinytest.add('numtel:mysql - initUpdateTable', function(test){
  var table = db._updateTable;

  test.isTrue(expectResult(db.queryEx(function(esc, escId){
